...
// waits for the responder to complete the transfer.
  ...
  // -------------------------------------------------
  // Build address byte {addr, rw}
  // -------------------------------------------------
  addr_byte = {initiator_trans.addr, (initiator_trans.op == I2C_RD)};
  ...
  // WRITE operation
  // -------------------------------------------------
  if (initiator_trans.op == I2C_WR) begin

    foreach (initiator_trans.data[j]) begin

      for (i = 7; i >= 0; i--) begin
        @(negedge scl_i);
        sda_o <= initiator_trans.data[j][i];
        @(posedge scl_i);
      end
  ...
  // -------------------------------------------------
    ...
    foreach (initiator_trans.data[j]) begin
      ...
      initiator_trans.data[j] = 0;

      for (i = 7; i >= 0; i--) begin
        @(posedge scl_i);
        initiator_trans.data[j][i] = sda_i;
      end

      // ACK intermediate bytes and NACK the final byte so the slave
      // terminates the read cleanly.
      @(negedge scl_i);
      if (j == (initiator_trans.data.size()-1))
        sda_o <= 1;
      else
        sda_o <= 0;
      @(posedge scl_i);
